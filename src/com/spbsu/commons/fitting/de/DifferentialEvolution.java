 ...
 * @author vp
 */
public class DifferentialEvolution extends MinimizationAlgorithm {
  ...
  public static double WEIGHT_FACTOR = 0.7 /* 0.5*/;

  /** Crossing over factor (default 0.9) */
  public static double CROSS_OVER_FACTOR = 0.9 /*1.0*/;
   ...
   * <p/>
   * <p><em>DE web page:</em>
   * <a href="http://www.icsi.berkeley.edu/~storn/code.html" >http://www.icsi.berkeley.edu/~storn/code.html</a>
   *
   * @param dimension dimension of optimization vector
   */
  public DifferentialEvolution(final int dimension) {
    this(dimension, 5 * dimension);
  }
  ...
  public DifferentialEvolution(int dimension, int populationSize) {
    ...
    // Allocate memory
    currentPopulation = new double[this.populationSize][this.dimension];
    nextPopulation = new double[this.populationSize][this.dimension];
    costs = new double[this.populationSize];
    trialVector = new double[this.dimension];
...
//    numr = 3; // for stragey DE/rand-to-best/1/bin
  ...
  public void optimize(MultivariateFunction func, double[] xvec, double tolfx, double tolx, MinimizationMonitor monitor) {
    f = func;
    x = xvec;
    ...
    while (true) {
      ...
      do {
        ...
        if (printMode > 1 && currGen % 20 == 0) {
          printStatistics();
        }

        if (monitor != null && xHasChanged) {
    ...
    if (printMode > 0) printStatistics();
  }
  ...
  private void firstGeneration() {
    ...
    for (int i = 0; i < populationSize; i++) {
      for (int j = 0; j < dimension; j++) {
        ...
        currentPopulation[i][j] = min + diff * nextDouble();
      }
  ...
  private boolean nextGeneration() {
    ...
    for (int r0 = 0; r0 < populationSize; r0++) {
      ...
      for (int i = 0; i < dimension; i++) // perform binomial trials
      {
        ...
        if (nextDouble() < CROSS_OVER_FACTOR || i == dimension - 1) {
          // DE/rand-to-best/1/bin
          // (change to 'numr=3' in constructor when using this strategy)
//          trialVector[n] = trialVector[n] +
//            WEIGHT_FACTOR * (x[n] - trialVector[n]) +
//            WEIGHT_FACTOR * (currentPopulation[r[1]][n] - currentPopulation[r[2]][n]);
          ...
          trialVector[n] = x[n] +
          	 ...
          	 -currentPopulation[r[3]][n]-currentPopulation[r[4]][n])*WEIGHT_FACTOR;
        }
        n = (n + 1) % dimension;
      }
  ...
  private void printStatistics() {
    ...
    double varCost = 0.0;
    for (int i = 0; i < populationSize; i++) {
      final double tmp = (costs[i] - meanCost);
      varCost += tmp * tmp;
    }
    ...
    System.out.println("Weight factor (WEIGHT_FACTOR): " + WEIGHT_FACTOR);
    System.out.println("Crossing-over (CROSS_OVER_FACTOR): " + CROSS_OVER_FACTOR);
    System.out.println();
  }
