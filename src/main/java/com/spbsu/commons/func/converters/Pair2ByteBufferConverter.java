...
public class Pair2ByteBufferConverter<A, B> implements Converter<Pair<A, B>, ByteBuffer> {
  ...
  public Pair2ByteBufferConverter(final Converter<A, ByteBuffer> converterA, final Converter<B, ByteBuffer> converterB) {
    this.converterA = converterA;
    this.converterB = converterB;
  }

  @Override
  public Pair<A, B> convertFrom(final ByteBuffer source) {
    return Pair.create(converterA.convertFrom(source), converterB.convertFrom(source));
  }

  @Override
  public ByteBuffer convertTo(final Pair<A, B> object) {
